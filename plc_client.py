...
PLC_IP = '127.0.0.1'
PLC_RACK = 0
PLC_SLOT = 1  # S7-1200/1500: slot=1; S7-300: slot=2; S7-400: slot=3
...
if not connected:
    ...
    print()
    print("  3. DB1 数据块是否存在且非优化访问？")
    print("     右键 DB1 → 属性 → 取消「优化的块访问」")
    print()
    print(f"  4. 最后异常信息: {last_error}")
    print("=" * 55)
    exit(1)

DB_NUMBER = 1  # 对应博途里的 DB1
...
try:
    ...
    print(f"[诊断] 读失败: {e}")
    print()
    print("  ⚠ 读取也失败 → 确认是 DB1「优化的块访问」未关闭！")
    print("  操作步骤：")
    print("    1. 博途中右键 DB1 → 属性")
    print("    2. 取消勾选「优化的块访问」(Optimized block access)")
    print("    3. 重新编译 DB1（右键 → 编译 → 软件(全部重建)）")
...
try:
    while True:
        ...
        # 准备一个长度为 10 字节的空白内存块 (因为你的 DB 块偏移量到了 8.0，再加上一个 Int(2字节) = 10)
