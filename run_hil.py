    ...
    try:
        for step in range(args.steps):
            ...
            logger.info(
                ...
                f"q_f={action[0]:5.2f} q_a={action[1]:5.2f} | "
                f"PLC: F={plc_state.get('Actual_Valve_F', 0):5.3f} "
                f"A={plc_state.get('Actual_Valve_A', 0):5.3f} "
                f"CommOK={plc_state.get('Remote_Comms_OK')} "
                f"Alarm={plc_state.get('System_Alarm_Light')} | "
                f"Sim: θ={info.get('theta', 0):.3f} EC={info.get('ec_soil', 0):.3f}"
            )
